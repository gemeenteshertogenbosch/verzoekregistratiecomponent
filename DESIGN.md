...
It is conceivable that in future iterations we would like to use indexed array in the following situations: when the index of the array can't be assumed on basis of url notation, when indexes arn't numerical, when we do not want an index to start at 0 or when indexes are purposly missing. Comma notation of id,name,description would always refer to te equivalent of fields: [
  0 => id,
  1 => name,
  2 => description
]
